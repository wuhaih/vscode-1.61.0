...
function getElectron(arch: string): () => NodeJS.ReadWriteStream {
	return () => {
		//你可以通过两种不同的方式使用这个模块：打包你的应用程序和/或 下载一个 Electron 版本到磁盘。
		//如果您使用gulp.dest 管道将构建的应用程序传输到磁盘，则这些符号链接将丢失。symdest将确保考虑符号链接。
...
async function main(arch = process.arch): Promise<void> {
	...
	// }
	// 读取elctron版本 .yarnrc文件
	const version = util.getElectronVersion();
	// 构建electron生成的文件路径
	const electronPath = path.join(root, '.build', 'electron');
	// electron版本号
	const versionFile = path.join(electronPath, 'version');
	//判断换存的electron与.yarnrc版本是否一致
	const isUpToDate = fs.existsSync(versionFile) && fs.readFileSync(versionFile, 'utf8') === `${version}`;
	//不一致则从远程获取对应的electron版本
	if (!isUpToDate) {
		await util.rimraf(electronPath)();
		console.log('------------await util.streamToPromise(getElectron(arch)())-------------', arch)
		await util.streamToPromise(getElectron(arch)());
	}
}
